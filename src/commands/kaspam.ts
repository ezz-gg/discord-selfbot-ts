...
export async function run(Client: Discord.Client, message: Discord.Message,  args: any[]) {
    ...
    if (kaisu === 2 || 2 < kaisu) {
      ...
      console.log("kaspamを停止しました");
    } else console.log("kaspamを開始しました");
    while (kaisu === 1) {
      let rand = Math.floor(Math.random() * 100);
      if (rand < 30) {
        let kasomsg = "最近かそったな～";
        await message.channel.send(
          `${kanjilist[Math.floor(Math.random()*kanjilist.length)]}${kasomsg}${kanjilist[Math.floor(Math.random()*kanjilist.length)]}`
        );
      } else if (rand < 50) {
        let kasomsg = "過疎:sob:";
        await message.channel.send(
...
const info = {
    name: "kaspam",
    description: "Spam KASO not mention",
    category: "spam",
    args: "none"
}

...
import { Ownerid, Prefix, Only_guild } from "../envs";
...
export default async (Client: Discord.Client, message: Discord.Message) => {
	...
	if (message.guild.id === Only_guild) {
		if (message.content.startsWith(Prefix)) {
			if (message.author.id === Ownerid || !JSON.parse(fs.readFileSync(path.join(__dirname,'../blacklist.json'), 'utf8')).hasOwnProperty(message.author.id)) {
